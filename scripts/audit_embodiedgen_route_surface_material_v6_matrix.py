#!/usr/bin/env python3
"""Audit the frozen 6-scene x 2-material RTX development matrix."""
...
AUDIT_SCHEMA = "kinofail.embodiedgen-route-surface-material-v6-matrix-audit.v1"
...
def _cell(scene: dict[str, Any], material_id: str, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    root = corridor_path.parent.parent / "route_surface_material_v6_development" / material_id
        ...
        checks.update(
            {
                ...
                == scene["sha256"],
                "compiled_material_id": material.get("id") == material_id,
                "compiled_material_split": material.get("split") == "train",
                "compiled_material_cc0": material.get("license") == "CC0",
                "compiled_visual_only": appearance.get("visual_intervention_only") is True,
        ...
        checks.update(
            {
                ...
                "rtx_structural_checks_complete": len(structural) == 8
...
def main() -> None:
    ...
    evidence_checks = {
        name: _hash_spec(spec) for name, spec in plan["selection_evidence"].items()
    }
    scene_checks = {
    ...
    payload = {
        ...
        "cells": cells,
        "interpretation": (
            "Development-only cross-scene validation of train-split material candidates; "
            "no cell is benchmark confirmation evidence."
        ),
    }
