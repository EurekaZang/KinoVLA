        ...
        for rec in recs:
            ...
            snaps.append(
                Snap(
                    ...
                    cell=cell,
                    t3_sub=_t3_sub(rec),
                    truth=rec["ground_truth"]["category"],
                    admissible=frozenset(rec.get("admissible_recovery_set") or []),
                    mu=rec.get("a3_mu"),
...
def score_agent(policy, snaps: list[Snap]) -> list[dict]:
    rows = []
    for s in snaps:
        ...
        rows.append({
            "sid": s.sid, "cell": s.cell, "t3_sub": s.t3_sub, "truth": s.truth,
            "attribution": attr, "primitive": prim, "attr_ok": attr_ok,
            "mu": s.mu, "parsed": parsed,
        })
    return rows
...
def main() -> None:
    ...
    if args.recompute:  # rebuild the heatmap/dose from cached per_item_*.json (no agent runs)
        for a in agents:
            ...
            if f.exists():
                per_agent[a] = json.loads(f.read_text())
    ...
    if not args.recompute and "B1" in agents:
        ...
        per_agent["B1"] = score_agent(b1, snaps)
    ...
    if not args.recompute and "B-F" in agents:
        ...
        per_agent["B-F"] = score_agent(bf, snaps)
    ...
    if not args.recompute and vla:
        ...
        for name, route, adapter, mask in runs:
            ...
            pol = ModelVlaPolicy(m, pcfg, tax, route=route, n_images=ni, temperature=0.0,
                                 proprio_detail=pdet, mask_proprio=mask)
            per_agent[name] = score_agent(pol, snaps)
    ...
    if args.only_cell:
        for a in list(per_agent):
            ...
            if cached.exists():
                ...
                print(f"[a3] merged {a}: {len(keep)} cached non-{args.only_cell} + "
                      f"{len(per_agent[a]) - len(keep)} fresh {args.only_cell}", flush=True)

    # ---- heatmap ----
    heatmap = {a: {c: cell_acc(per_agent[a], c) for c in CELLS} for a in agents}
    t3_detail = {a: {s: _acc([r for r in per_agent[a] if r["t3_sub"] == s])
                     for s in ("looks_safe", "O8", "reverse")} for a in agents}
    ...
    for a in agents:
        ...
        for mu in sorted(mu_set, reverse=True):
            ...
            per_mu[str(mu)] = {"n": len(rs), "p_low_friction": round(k / max(1, len(rs)), 3),
                               "ci": wilson(k, len(rs))}
        dose[a] = per_mu
    ...
    result = {
        ...
        "t3_dir": args.t3, "commit": manifest_a03.get("commit"),
        "n_snaps": len(snaps), "per_cell_n": by_cell,
        "agents": agents, "heatmap": heatmap, "t3_detail": t3_detail, "dose_response": dose,
    ...
    for a in agents:
        (out / f"per_item_{a.replace('-','_')}.json").write_text(json.dumps(per_agent[a], indent=2))
...
def _render_figures(out: Path, agents, heatmap, dose, per_agent) -> None:
    ...
    for i, a in enumerate(agents):
        for j, c in enumerate(CELLS):
            cell = heatmap[a][c]
            ax.text(j, i, f"{cell['acc']:.2f}\nn={cell['n']}", ha="center", va="center",
                    fontsize=8, color="black")
    ax.set_title("A3 taxonomy × agents — attribution accuracy\n"
    ...
    ax.set_title("A3.4 override dose-response (O7 looks_safe)\n"
