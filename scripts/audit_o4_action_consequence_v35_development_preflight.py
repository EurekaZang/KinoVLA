#!/usr/bin/env python3
"""Fail-closed preflight for the frozen v35 adaptive-Backstep development cohort."""
...
def _v35_runtime_delta_is_exact(
...
def _phase_aware_contract_is_exact(contract: dict[str, Any]) -> bool:
    return contract == {
        "route_budget_m": 0.3,
...
def main() -> int:
    ...
    checks: dict[str, bool] = {
        ...
        == "kinofail.o4-action-consequence-v35-development-freeze.v1",
        "frozen_before_first_execution": config.get("freeze_status")
        == "frozen_before_first_v35_development_execution",
        ...
        "v35_runtime_delta_exact": runtime_source_path.is_file()
        and _sha256(runtime_source_path) == config["runtime_source"]["sha256"]
        and _v35_runtime_delta_is_exact(
            runtime_source.get("runtime", {}), config.get("runtime", {}), collector_sha256
        ),
        "adaptive_architecture_delta_exact": config.get("architecture_delta") == {
            "emergency_backstep_speed_mps": {"v32": None, "v35": 0.48},
            "maximum_postdecision_forward_penetration_m": {"v32": None, "v35": 0.025},
            "trigger_inputs": "body_state_only_no_scene_identity_or_adhesion_release",
            "latching": True,
        },
        "phase_aware_route_contract_frozen": _phase_aware_contract_is_exact(
            config.get("phase_aware_route_contract", {})
        ),
        "state_aware_visual_contract_inherited": config.get("visual_qa_contract")
        == runtime_source.get("visual_qa_contract"),
    ...
    for case in cases:
        ...
        compiled = _json(_resolve(prerequisites.get("terrain_compiled_audit", {}).get("path", "__missing__")))
    ...
    result = {
        "schema_version": "kinofail.o4-action-consequence-v35-development-preflight.v1",
