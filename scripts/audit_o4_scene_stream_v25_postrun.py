#!/usr/bin/env python3
"""Seal the first terminal prefix of the v25 realistic-scene stream."""
...
def main() -> int:
    ...
    for scene in config["candidate_stream"]:
        ...
        artifacts = {
            ...
            "motion_proxy": _artifact(route / pipeline["motion_proxy_output_name"] / "route_motion_rtx_audit.json"),
            ...
            "go2_launcher": _artifact(terrain / "go2_qa/go2_launcher_audit.json"),
    ...
    for index, row in enumerate(rows):
        ...
        if admitted_count >= policy["minimum_admitted_scenes"] and len(admitted_families) >= policy["minimum_admitted_families"]:
            first_quota_prefix = index + 1
            break
    quota_reached = first_quota_prefix is not None
    exact_stop = (quota_reached and processed_count == first_quota_prefix) or (
    ...
    checks = {
        ...
        "processed_prefix_is_terminal": bool(processed_rows) and all(row["terminal"] for row in processed_rows),
        "all_processed_source_identities_match": all(row["source_identity_matches"] for row in processed_rows),
    ...
    result = {
        "schema_version": "kinofail.o4-scene-stream-v25-postrun.v1",
