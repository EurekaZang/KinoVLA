#!/usr/bin/env python3
"""Collect train-only O4 Continue/Backstep consequences from a matched decision state."""
...
def main() -> int:
    ...
    parser.add_argument("--backstep-steps", type=int, default=50)
    ...
    if args.horizon_steps <= 0 or args.decision_dwell_steps < 1 or args.backstep_steps < 1:
    ...
    try:
        ...
        appearance = appearance_binding_from_record(
            {
                ...
                "appearance_id": f"v20-o4-action-{args.material_id}",
                "uv_scale": 1.0,
                "uv_rotation_deg": 0.0,
                "surface_state": "damp",
            },
        ...
        for take in ("continue", "backstep"):
            ...
            predecision_step: int | None = reference_predecision_step
            ...
            for step in range(args.horizon_steps):
                if (
                    ...
                    and predecision_step < step <= predecision_step + args.backstep_steps
                ):
                    speed = -args.backstep_speed_mps
                    action_phase = "backstep"
                elif (
                    take == "backstep"
                    and predecision_step is not None
                    and step > predecision_step + args.backstep_steps
                ):
                    speed = 0.0
                ...
                if first_attachment_step is None and any(
                    ...
                    discovered = step + args.decision_dwell_steps
                    if take == "continue":
                        predecision_step = discovered
                        reference_predecision_step = discovered
                    elif discovered != reference_predecision_step:
                        raise RuntimeError(
                            "backstep first attachment does not reproduce the reference decision step"
                        )
                if predecision_step is not None and step == predecision_step:
        ...
        manifest = {
            "schema_version": "kinofail.o4-action-consequence.v20",
            ...
            "decision_contract": {
                "trigger": "first named-foot attachment plus fixed dwell",
                "decision_dwell_steps": args.decision_dwell_steps,
                ...
                "backstep_action": (
                    f"reverse {args.backstep_speed_mps} m/s for {args.backstep_steps} steps, "
                    "then hold"
                ),
