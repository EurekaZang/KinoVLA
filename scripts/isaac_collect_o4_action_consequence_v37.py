#!/usr/bin/env python3
"""Collect v37 velocity-damped adaptive O4 Continue/Backstep consequences.

The adhesive contact law remains command-agnostic and the v32 one-step branch remains frozen.
Backstep retains the v35 scene-identity-agnostic penetration/urgency guard.  Once emergency mode
latches, a second route controller adds measured lateral-velocity damping and greater lateral
authority so that reversal momentum cannot outrun cross-track correction.
"""
...
def main() -> int:
    ...
    parser.add_argument("--emergency-cross-track-gain-per-s", type=float, default=2.0)
    parser.add_argument(
        "--emergency-lateral-velocity-damping", type=float, default=1.0
    )
    parser.add_argument("--emergency-lateral-limit-mps", type=float, default=0.4)
    ...
    if (
        ...
        or args.emergency_cross_track_gain_per_s <= 0.0
        or args.emergency_lateral_velocity_damping <= 0.0
        or args.emergency_lateral_limit_mps < args.lateral_limit_mps
    ):
    ...
    try:
        ...
        emergency_controller = DampedRouteControllerV3(
            cross_track_gain_per_s=args.emergency_cross_track_gain_per_s,
            lateral_velocity_damping=args.emergency_lateral_velocity_damping,
            heading_gain_per_s=args.heading_gain_per_s,
            lateral_limit_mps=args.emergency_lateral_limit_mps,
            yaw_rate_limit_radps=args.yaw_rate_limit_radps,
        )
        ...
        for take in ("continue", "backstep"):
            ...
            for step in range(args.horizon_steps):
                ...
                selected_controller = (
                    ...
                    if action_phase == "backstep_emergency_clearance"
                    else controller
                )
                command, control = selected_controller.command(
                    frame,
        ...
        manifest = {
            "schema_version": "kinofail.o4-action-consequence.v37",
            ...
            "decision_contract": {
                ...
                "emergency_route_control": {
                    "cross_track_gain_per_s": args.emergency_cross_track_gain_per_s,
                    "lateral_velocity_damping": (
                        args.emergency_lateral_velocity_damping
                    ),
                    "lateral_limit_mps": args.emergency_lateral_limit_mps,
            ...
            "route_controller": {
                "nominal_controller_contract": controller.contract(),
                "emergency_controller_contract": emergency_controller.contract(),
