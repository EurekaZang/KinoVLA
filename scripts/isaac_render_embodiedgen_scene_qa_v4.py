#!/usr/bin/env python3
"""RTX QA for render-only route-surface and lighting-v4 development scenes."""
...
def main() -> int:
    ...
    try:
        ...
        if not all(structural_checks.values()):
            raise RuntimeError(f"v4 structural checks failed: {structural_checks}")
        ...
        cam = Camera(
            prim_path="/World/KinoRouteSurfaceQACamera",
            position=first_eye,
        ...
        for view in views:
            ...
            print(f"[rtx-v4] {view['name']}: {metrics}", flush=True)
        ...
        visual_checks = evaluate_pitch_aware_metrics(records, canonical_pairwise_l1)
        sheet_path = args.out / "route_surface_contact_sheet.png"
        _contact_sheet(paths, sheet_path)
        audit = {
            "schema_version": "kinofail.embodiedgen-rtx-scene-qa.v4-development",
            ...
            "admission_state": (
                "route_surface_lighting_v4_passed_development_only"
                if all(structural_checks.values()) and all(visual_checks.values())
                else "route_surface_lighting_v4_failed_development"
            ),
