#!/usr/bin/env python3
"""Freeze an excluded O9 engagement-branch regression diagnostic D7."""
...
D6 = ROOT / "outputs/kinofail_reconfirmation_a4_v7_checkpoint_tuning_d6/design_audit.json"
OUTPUT = ROOT / "outputs/kinofail_reconfirmation_a4_v7_checkpoint_tuning_d7"
...
def main() -> int:
    ...
    for path in (SOURCE, D6, COLLECTOR, PRE, POLICY, MANIFEST, FREEZE, SIM):
        if not path.is_file():
            raise FileNotFoundError(path)
    rows = [
        json.loads(line)
        for line in SOURCE.read_text().splitlines()
        if line and json.loads(line)["operator"] == "O9_high_centering"
    ]
    if len(rows) != 1:
    ...
    row.update(
        {
            "pairing": "outcome-exposed O9 engagement-branch regression D7",
    ...
    protocol = {
        "schema_version": "kinofail.reconfirmation-a4-v7-checkpoint-tuning-d7.v1",
        "protocol_id": "kinofail-reconfirmation-a4-v7-checkpoint-tuning-d7",
        ...
        "regression_target": {
            "defect": "O9 engagement was shadowed by the O5 payload branch",
            "expected": "O9 uses audited sustained belly-contact semantics",
        },
        "d6_design_sha256": sha256(D6),
    }
