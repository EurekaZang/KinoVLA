#!/usr/bin/env python3
"""Freeze the A1 bounded-RTX throughput amendment before simulation."""
...
def main() -> None:
    ...
    collector = ROOT / "scripts/isaac_collect_kinofail_realistic_a1_matched_v2.py"
    v1 = ROOT / "configs/data/kinofail_realistic_a1_matched_formal_v1.json"
    output = ROOT / "configs/data/kinofail_realistic_a1_matched_formal_v2.json"
    if output.exists():
    ...
    protocol = {
        ...
        "protocol_id": "kinofail_realistic_a1_matched_o2_o4_36_v2",
        "status": "frozen",
        ...
        "supersedes_uncollected_protocol": str(v1.relative_to(ROOT)),
        "superseded_protocol_sha256": _sha(v1),
        "amendment_reason": "Throughput only: retain full physics/proprio/telemetry but bound 10 Hz three-view RTX to 12 frames beginning at route progress 0.45 m; the fixed operator region begins at 0.70 m. No v1 manifest was produced.",
        ...
        "collection_contract": {
            ...
            "full_physics_horizon_retained": True,
