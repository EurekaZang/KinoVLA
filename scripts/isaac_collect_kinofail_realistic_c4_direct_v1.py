#!/usr/bin/env python3
"""Collect the frozen realistic C4 selective-versus-always-safe consequences.

The deployed decision is precomputed from the hash-bound RGB/proprio snapshot and
is replayed unchanged in both branches.  This collector never computes a decision
from operator identity or outcome data: privileged fields are used only to install
the benchmark intervention and score its physical consequence.
"""
...
def _operator_engaged(operator: str, telemetry: dict[str, Any]) -> bool:
    ...
    if operator == "O8_invisible_collider":
        return bool(telemetry.get("enabled"))
...
def _decision_ready(
    ...
    if operator == "O5_payload":
        return time_s >= 0.5 and _operator_engaged(operator, telemetry)
    if operator == "O8_invisible_collider":
...
def _terminal_success(
    ...
    *,
    ...
    if executed_action == "hold_and_request":
        return final_speed_mps < 0.08
    return final_progress_m >= 2.0
...
def main() -> int:
    ...
    preliminary.add_argument("--policy", required=True)
    ...
    parser.add_argument("--policy", default=pre.policy)
    ...
    try:
        ...
        from scripts.isaac_collect_kinofail_realistic_pair_v1 import (
            _install_operator,
            _telemetry,
        )
        ...
        expected_hashes = {
            ...
            "shared_actor_sha256": _sha(policy_path),
        }
        for key, actual in expected_hashes.items():
            if protocol.get(key) != actual:
                raise RuntimeError(
                    f"protocol {key} mismatch: {protocol.get(key)} != {actual}"
                )
        ...
        if len(cases) != expected_per_scene:
            raise RuntimeError(
                f"expected {expected_per_scene} cases for scene, got {len(cases)}"
            )
        registry = _json(registry_path)
        registry_rows = [
            row for row in registry["scenes"] if row["scene_id"] == args.scene
        ]
        if len(registry_rows) != 1:
            raise RuntimeError("scene registry binding is not unique")
        scene_row = registry_rows[0]
        if str(scene_row["split"]) != "test":
            raise RuntimeError("direct C4 primary corpus is restricted to test scenes")
        episode_path = (ROOT / scene_row["episode_usd"]).resolve()
        compiled_path = (ROOT / scene_row["compiled_audit"]).resolve()
        if (
            _sha(episode_path) != scene_row["episode_sha256"]
            or _sha(compiled_path) != scene_row["compiled_audit_sha256"]
        ):
        ...
        controller = DampedRouteControllerV3(
            ...
            lateral_limit_mps=0.50,
            yaw_rate_limit_radps=0.80,
        )
        ...
        if args.resume and result_path.exists():
            for row in _jsonl(result_path):
                prior[(str(row["case_id"]), str(row["branch"]))] = row
        ...
        for case in cases:
            ...
            frozen_decision = dict(case["decision"])
            pair_rows: dict[str, dict[str, Any]] = {}
            for branch in ("selective", "always_safe"):
                key = (str(case["case_id"]), branch)
                if key in prior:
                    pair_rows[branch] = prior[key]
                    continue
                executed_action = (
                    str(frozen_decision["recovery_action"])
                    if branch == "selective" and bool(frozen_decision["release"])
                    else str(frozen_decision["safe_action"])
                )
                seed = int(case["reset_seed"])
                obs = backend.deep_reset(seed)
                operator, _region = _install_operator(backend, record, frame, seed)
                engaged_dwell = 0
                ...
                for step in range(args.horizon_steps):
                    ...
                    if post:
                        if executed_action == "backstep_release":
                        ...
                        elif executed_action == "hold_and_request":
                            speed = 0.0
                            action_phase = "hold_and_request"
                        else:
                            raise RuntimeError(
                                f"unknown frozen C4 action: {executed_action}"
                            )

                    backend.set_posture(None, 1.0)
                    ...
                    if obs.fallen:
                        break
                    if post and step - decision_step >= 160:
                        break

                if decision_step is None:
                    raise RuntimeError(
                        f"operator never reached decision onset: "
                ...
                if not np.isfinite(min_post_progress):
                    min_post_progress = float(progress)
                o4_active = (
                    int(final_telemetry.get("active_feet", 0))
                ...
                result = {
                    "schema_version": "kinofail.realistic-c4-direct-outcome.v1",
                    ...
                    "truth_attribution": case["truth_attribution"],
                    "source_sample_id": case["source_sample_id"],
                    "source_physical_episode_id": case[
                    ...
                    "decision_progress_m": decision_progress,
                    "predecision_rows_sha256": _prefix_hash(
                        rows, decision_step
                    ),
                    "branch_started_strictly_after_decision": True,
                    "shared_actor_sha256": _sha(policy_path),
                    "operator_engaged": _operator_engaged(
                        operator_name, final_telemetry
                    ),
                    "steps": len(rows),
                    "fell": bool(obs.fallen),
            ...
            if len(pair_rows) == 2:
                ...
                if values[0]["decision_step"] != values[1]["decision_step"]:
                    raise RuntimeError(
                        f"paired decision step mismatch: {case['case_id']}"
                    )
                if (
                    values[0]["predecision_rows_sha256"]
                    != values[1]["predecision_rows_sha256"]
                ):
                    raise RuntimeError(
                        f"paired predecision hash mismatch: {case['case_id']}"
                    )

        expected = {
            (str(row["case_id"]), branch)
            for row in cases
            for branch in ("selective", "always_safe")
        }
        observed = {
            (str(row["case_id"]), str(row["branch"])) for row in written
        }
        complete = expected <= observed
        summary = {
            "schema_version": "kinofail.realistic-c4-direct-scene-summary.v1",
            "scene_cluster": args.scene,
            "protocol_id": protocol["protocol_id"],
            "passed": complete,
            "expected_episodes": len(expected),
            "completed_episodes": len(expected & observed),
            "weak_null_and_unfavorable_outcomes_retained": True,
        }
