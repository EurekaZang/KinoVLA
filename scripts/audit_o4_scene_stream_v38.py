#!/usr/bin/env python3
"""Audit v38 operator-blind scene acquisition for v37 development only."""
...
EXPECTED_FREEZE_STATUS = (
    "frozen_before_any_v38_development_candidate_generation_or_o4_execution"
)
...
def _preflight(config_path: Path, out: Path) -> int:
    ...
    checks: dict[str, bool] = {
        "supported_v38_wrapper_schema": raw.get("schema_version")
        ...
        and evidence.get("successful_v37_development_requires_a_later_fresh_heldout_cohort")
        is True,
        ...
        is True,
        "all_admitted_enter_v37_development": policy.get(
            "all_admitted_scenes_must_enter_v37_development"
        )
        is True,
    ...
    result = {
        "schema_version": "kinofail.o4-scene-stream-v38-preflight.v1",
...
def _patch_postrun(out: Path) -> None:
    ...
    payload["schema_version"] = "kinofail.o4-scene-stream-v38-postrun.v1"
    payload["audit_adapter"] = "v38_six_scene_v37_development_quota"
    payload["v30_audit_implementation_reused"] = True
    payload["v37_development_authorized"] = payload.get("passed") is True
    payload["heldout_confirmation_authorized"] = False
    payload["next_gate"] = (
        "Freeze v37 against every admitted development scene and execute each exactly once. "
