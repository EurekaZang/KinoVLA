#!/usr/bin/env python3
"""Freeze the final O9 semantic-repair-v3 collection contract."""
...
SCHEDULE = (
    ROOT / "outputs/kinofail_realistic/design_o9_semantic_repair_v3/schedule.jsonl"
)
DESIGN_AUDIT = (
    ROOT
    / "outputs/kinofail_realistic/design_o9_semantic_repair_v3/design_audit.json"
)
SEMANTIC_AUDIT = (
...
PILOTS = [
    ...
    ROOT
    / "outputs/kinofail_realistic/corpus_o9_semantic_repair_v2/"
    "pair_summaries/cf_126a04c6a2399393cdc4.json",
]
COLLECTOR = (
    ROOT / "scripts/isaac_collect_kinofail_realistic_o9_semantic_repair_v3.py"
)
...
PROTOCOL = (
    ROOT / "configs/data/kinofail_realistic_o9_semantic_repair_formal_v3.json"
)
CORPUS = ROOT / "outputs/kinofail_realistic/corpus_o9_semantic_repair_v3"
BENCHMARK_ID = "kinofail_realistic_o9_semantic_repair_v3"
...
def main() -> None:
    ...
    if existing:
        raise RuntimeError(
            "refusing to freeze after O9 semantic-repair-v3 outcomes exist: "
    ...
    if design.get("status") != "passed" or design.get("benchmark_id") != BENCHMARK_ID:
        raise RuntimeError("v3 design audit mismatch")
    pilot_locks = [
    ...
    protocol = {
        ...
        "protocol_id": "kinofail_realistic_o9_semantic_repair_v3",
        ...
        "design_provenance": {
            ...
            "excluded_pilot_locks": pilot_locks,
            "pilot_disposition": (
                "v1 excluded for nominal route departure at legacy speed; v2 "
                "excluded because 18.3 N sustained base load missed the frozen "
                "20 N threshold; neither pilot enters training or evaluation"
            ),
            "model_predictions_used_to_choose_geometry_thresholds_or_speed": False,
            "v3_outcomes_present_at_freeze": False,
        },
    }
