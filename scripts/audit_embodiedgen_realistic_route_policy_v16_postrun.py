#!/usr/bin/env python3
"""Adjudicate the failure-closed v16 unseen nominal-confirmation batch."""
...
SCHEMA_VERSION = "kinofail.embodiedgen-realistic-route-policy-v16-postrun.v1"
...
def main() -> int:
    ...
    for request in config["requested_scenes"]:
        ...
        route = source / config["scene_pipeline"]["route_surface_output_name"] / request["material_id"]
        ...
        artifacts = {
            ...
            "base_compile": _artifact(source / config["scene_pipeline"]["base_output_name"] / "compiled_scene_audit.json"),
            "corridor_v2": _artifact(source / config["scene_pipeline"]["corridor_output_name"] / "compiled_scene_audit.json"),
            ...
            "stack_admission": _artifact(terrain / "realistic_stack_v16_admission.json"),
    ...
    sentinels = {
        item["scene_id"]: _artifact(_resolve(item["nominal_manifest"]["path"]))
        for item in config["development_sentinels"]
    }
    ...
    checks = {
        ...
        "two_development_sentinels_passed": len(sentinels) == 2
        and all(artifact["passed"] is True for artifact in sentinels.values()),
    }
    ...
    result = {
        ...
        "development_sentinels": sentinels,
        "scenes": rows,
        "evidence_boundary": {
        ...
        json.dumps(
            {
                ...
                "passed": confirmation_passed,
                "scene_admission": result["scene_admission"],
            },
            indent=2,
        )
    )
