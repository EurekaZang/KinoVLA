#!/usr/bin/env python3
"""Freeze the O9 semantic-repair protocol before collecting repair outcomes."""
...
COLLECTOR = (
    ROOT / "scripts/isaac_collect_kinofail_realistic_o9_semantic_repair_v1.py"
)
...
PROTOCOL = (
    ROOT / "configs/data/kinofail_realistic_o9_semantic_repair_formal_v1.json"
)
CORPUS = ROOT / "outputs/kinofail_realistic/corpus_o9_semantic_repair_v1"
BENCHMARK_ID = "kinofail_realistic_o9_semantic_repair_v1"
...
def main() -> None:
    ...
    if existing:
        raise RuntimeError(
            "refusing to freeze after O9 semantic-repair outcomes exist: "
            f"{len(existing)} pair summaries"
        )
    rows = jsonl(SCHEDULE)
    audit = json.loads(DESIGN_AUDIT.read_text(encoding="utf-8"))
    semantic_audit = json.loads(SEMANTIC_AUDIT.read_text(encoding="utf-8"))
    if audit.get("status") != "passed":
        raise RuntimeError("design audit is not passed")
    if semantic_audit.get("status") != "o9_semantics_failed_repair_required":
        raise RuntimeError("source O9 semantic audit does not require repair")

    pair_ids = sorted({row["counterfactual_group_id"] for row in rows})
    protocol = {
        "schema_version": "kinofail.formal-collection-protocol.v1",
        "protocol_id": "kinofail_realistic_o9_semantic_repair_v1",
        "benchmark_id": BENCHMARK_ID,
        "status": "frozen",
        "frozen_utc": datetime.now(timezone.utc).isoformat(),
        "scope": (
            "Targeted replacement of all 90 scale-v8 O9 pairs after a "
            "telemetry-only semantic audit showed that region exposure did not "
            "establish belly-on-crossbar high-centering."
        ),
        ...
        "design_provenance": {
            ...
            "repair_selected_from": (
                "body-link contact telemetry and O9 mechanism definition only"
            ),
            "model_predictions_used_to_choose_geometry_or_thresholds": False,
            "repair_outcomes_present_at_freeze": False,
        },
    }
