#!/usr/bin/env python3
"""Collect C2 v3 T3 with one isolated Isaac process per physical pair."""
...
def main() -> int:
    ...
    if not groups or not all(
        len(values) == 2
        and {str(row["condition"]) for row in values}
        == {"nominal_counterfactual", "anomaly"}
        for values in groups.values()
    ):
        raise RuntimeError("C2 v3 isolated T3 selection is invalid")
    ordered = sorted(
        groups,
        key=lambda group_id: (
            ...
            json.dumps(
                {
                    "progress": f"{index}/{len(ordered)}",
                    "counterfactual_group_id": group_id,
                    "scene_cluster": representative["scene_family"],
