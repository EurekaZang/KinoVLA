#!/usr/bin/env python3
"""Seal every v26 held-out O4 Continue/Backstep result without scene selection."""
...
def _audit_case(case: dict[str, Any], runtime: dict[str, Any], launcher: dict[str, Any]) -> dict[str, Any]:
    ...
    if complete:
        ...
        outcome = {
            ...
            "actions_diverge_only_after_decision": len(continue_rows) > branch_step and len(backstep_rows) > branch_step and continue_rows[branch_step]["action_phase"] == "matched_forward" and backstep_rows[branch_step]["action_phase"] == "backstep",
    ...
    integrity = {
        ...
        "launcher_manifest_hash_matches": manifest_path.is_file() and launcher.get("manifest_sha256") == _sha256(manifest_path),
        "manifest_identity_matches": manifest.get("schema_version") == "kinofail.o4-action-consequence.v20" and manifest.get("scene_id") == case["scene_id"] and manifest.get("room_family") == case["room_family"] and manifest.get("material_id") == case["material_id"] and manifest.get("seed") == case["runtime_seed"],
        ...
        "collector_hash_matches_v21": provenance.get("collector_sha256") == runtime["collector_sha256"],
...
def main() -> int:
    ...
    cases = [_audit_case(case, config["runtime"], launchers.get(case["scene_id"], {})) for case in config["cases"]]
    integrity_checks = {
        ...
        "runner_bound_to_config_and_preflight": runner.get("config_sha256") == _sha256(config_path) and runner.get("preflight_sha256") == _sha256(preflight_path),
        "every_frozen_case_attempted_in_order": runner.get("attempted_scene_ids") == [case["scene_id"] for case in config["cases"]] and runner.get("all_cases_attempted") is True,
        "every_case_terminal_with_manifest": runner.get("all_cases_terminal") is True and runner.get("all_manifests_present") is True,
    ...
    result = {
        "schema_version": "kinofail.o4-action-consequence-v26-heldout-postrun.v1",
        ...
        "interpretation": "This seals held-out realistic O4 operator consequences only. It does not complete any A0-A7 experiment and cannot be mixed with legacy-domain metrics.",
        "next_gate": "Use the sealed failure modes without retuning v26 scenes; then freeze O5-O11 realistic operator confirmation and the registry-bound realistic corpus before retraining and rerunning A0-A7.",
    }
