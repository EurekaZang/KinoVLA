#!/usr/bin/env python3
"""Execute the frozen v31 phase-conditioned O4 cohort exactly once."""
...
COLLECTOR = ROOT / "scripts/isaac_collect_o4_action_consequence_v27.py"
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("v31 preflight did not authorize this exact config")
    ...
    for index, case in enumerate(config["cases"]):
        ...
        command = [
            ...
            "v31-o4-launcher", str(ISAAC_SETUP), str(PYTHON), *native_args,
        ]
        completed = subprocess.run(command, cwd=ROOT, check=False)
        manifest_path = output / "pair_manifest.json"
        manifest = _json(manifest_path) if manifest_path.is_file() else {}
        row = {
            "schema_version": "kinofail.o4-action-consequence-v31-launcher-audit.v1",
    ...
    result = {
        "schema_version": "kinofail.o4-action-consequence-v31-runner-audit.v1",
        "created_utc": datetime.now(UTC).isoformat(),
