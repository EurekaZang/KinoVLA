#!/usr/bin/env python3
"""Audit one matched-topology realistic-route O3 development pair."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--evidence-role",
        choices=("development_calibration", "heldout_geometry_confirmation", "embodiedgen_calibration", "embodiedgen_unseen_scene_confirmation"),
        required=True,
    )
    ...
    scene_contract = audit_realistic_route_scene_contract(compiled)
    ...
    checks = {
        ...
        "scene_route_collision_contract_satisfied": scene_contract.get("passed") is True,
    ...
    result = {
        "schema_version": "kinofail.realistic-route-o3-pair-audit.v3-candidate",
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite pair audit: {output}")
