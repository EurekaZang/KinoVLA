#!/usr/bin/env python3
"""Audit v36 operator-blind scene acquisition for v35 development only."""
...
EXPECTED_FREEZE_STATUS = (
    "frozen_before_any_v36_development_candidate_generation_or_o4_execution"
)
...
def _preflight(config_path: Path, out: Path) -> int:
    raw = v30.predecessor._original_json(config_path)
    ...
    checks: dict[str, bool] = {
        "supported_v36_wrapper_schema": raw.get("schema_version")
        == "kinofail.o4-scene-stream-v30-freeze.v1"
        and raw.get("freeze_status") == EXPECTED_FREEZE_STATUS,
        ...
        and evidence.get("counts_as_realistic_corpus_evidence") is False
        and evidence.get("counts_as_o4_confirmation") is False
        and evidence.get("a8_in_scope") is False
        and evidence.get("successful_v35_development_requires_a_later_fresh_heldout_cohort")
        is True,
        ...
        "development_six_scene_quota": _quota_contract(policy),
        ...
        is True,
        "all_admitted_enter_v35_development": policy.get(
            "all_admitted_scenes_must_enter_v35_development"
        )
        is True,
    ...
    result = {
        "schema_version": "kinofail.o4-scene-stream-v36-preflight.v1",
...
def _patch_postrun(out: Path) -> None:
    ...
    payload["schema_version"] = "kinofail.o4-scene-stream-v36-postrun.v1"
    payload["audit_adapter"] = "v36_six_scene_v35_development_quota"
    payload["v30_audit_implementation_reused"] = True
    payload["v35_development_authorized"] = payload.get("passed") is True
    payload["heldout_confirmation_authorized"] = False
    payload["next_gate"] = (
        "Freeze v35 against every admitted development scene and execute each exactly once. "
        "Any architecture selected here must later pass a fresh operator-blind held-out cohort."
    )
