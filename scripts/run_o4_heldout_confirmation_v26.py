#!/usr/bin/env python3
"""Execute every frozen v26 O4 held-out case exactly once, without outcome filtering."""
...
COLLECTOR = ROOT / "scripts/isaac_collect_o4_action_consequence_v20.py"
...
def _collector_args(case: dict[str, Any], runtime: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        "--decision-dwell-steps", str(runtime["decision_dwell_steps"]),
        "--backstep-steps", str(runtime["backstep_steps"]),
...
def main() -> int:
    ...
    if preflight.get("passed") is not True or preflight.get("config_sha256") != _sha256(config_path):
        raise RuntimeError("v26 preflight did not authorize this exact config")
    ...
    for index, case in enumerate(config["cases"]):
        ...
        command = [
            ...
            "v26-o4-launcher", str(ISAAC_SETUP), str(PYTHON), *native_args,
        ]
        completed = subprocess.run(command, cwd=ROOT, check=False)
        manifest_path = output / "pair_manifest.json"
        manifest = _json(manifest_path) if manifest_path.is_file() else {}
        row = {
            "schema_version": "kinofail.o4-action-consequence-v26-launcher-audit.v1",
    ...
    result = {
        "schema_version": "kinofail.o4-action-consequence-v26-runner-audit.v1",
        "created_utc": datetime.now(UTC).isoformat(),
    ...
    print(json.dumps({"runner_audit": str(runner_path), **{k: result[k] for k in ("all_cases_attempted", "all_cases_terminal", "all_manifests_present")}}, indent=2))
