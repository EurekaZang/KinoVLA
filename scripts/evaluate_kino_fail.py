"""Evaluate KINO and reference methods on the registered KINO-Fail benchmark."""
...
def main() -> int:
    ...
    parser.add_argument("--method", choices=["all", "kino", *registry["methods"]], default="all")
    ...
    method = "gmu" if args.method == "kino" else args.method
    methods = list(registry["methods"]) if method == "all" else [method]
    if args.include_action and "gmu" not in methods:
        parser.error("--include-action requires --method kino, gmu, or all")
