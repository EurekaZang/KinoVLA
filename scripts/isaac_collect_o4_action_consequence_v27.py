#!/usr/bin/env python3
"""Collect v27 phase-conditioned O4 Continue/Backstep consequences.

The adhesive contact law remains command-agnostic.  Only the recovery policy changes from the
v21 fixed decision/fixed-duration primitive: a posture-urgency condition may branch before the
maximum dwell, and Backstep continues until it has both retreated and cleared active bonds.
"""
...
def main() -> int:
    ...
    try:
        ...
        for take in ("continue", "backstep"):
            ...
            for step in range(args.horizon_steps):
                if (
                    ...
                    action_phase = "backstep_clearance"
                elif (
                    take == "backstep"
                    and predecision_step is not None
        ...
        manifest = {
            "schema_version": "kinofail.o4-action-consequence.v27",
            ...
            "decision_contract": {
                ...
                "backstep_action": (
                    f"reverse {args.backstep_speed_mps} m/s until retreat reaches "
