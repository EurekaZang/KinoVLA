#!/usr/bin/env python3
"""Build the fresh, permanently excluded A4-v7 integration pilot P3."""
...
OUTPUT = ROOT / "outputs/kinofail_reconfirmation_a4_v7_pilot_p3"
...
P2_DESIGN = ROOT / "outputs/kinofail_reconfirmation_a4_v7_pilot_p2/design_audit.json"
...
def main() -> int:
    ...
    priors = [json.loads(P1_DESIGN.read_text()), json.loads(P2_DESIGN.read_text())]
    failures = [json.loads(P1_FAILURE.read_text()), json.loads(P2_FAILURE.read_text())]
    if not all(row.get("passed") is True and row.get("pilot_passed") is False for row in failures):
        raise RuntimeError("prior pilot failures were not preserved")
    ...
    excluded_scenes = set()
    for prior in priors:
        excluded_ids.update(map(str, prior["source_counterfactual_group_ids"]))
        excluded_scenes.add(str(prior["scene_id"]))
    ...
    if not eligible:
        raise RuntimeError("no fresh P3 scene remains")
    scene = eligible[0]
    rows = []
    for operator in OPERATORS:
        group_id = min(
            by_cell[(scene, operator)],
            key=lambda value: stable_key("a4-v7-pilot-p3", scene, operator, value),
        )
        original = anomaly[group_id]
        replacement_id = f25_replacement_by_original.get(group_id)
        source = (
            f33_by_source[group_id]
            if operator == "O9_high_centering"
            else f25_anomaly[replacement_id]
        ...
        rows.append(
            {
                ...
                "case_id": f"a4v7pilotp3__{scene}__{operator}__moderate",
                "scene_id": scene,
                ...
                "pairing": "excluded fresh-scene integration pilot P3",
    ...
    protocol = {
        "schema_version": "kinofail.reconfirmation-a4-v7-pilot-p3.v1",
        "protocol_id": "kinofail-reconfirmation-a4-v7-excluded-pilot-p3",
        ...
        "predecision_actor": predecision_actor,
        ...
        "prior_failure_audit_sha256": {
            "p1": sha256(P1_FAILURE),
            "p2": sha256(P2_FAILURE),
        },
    }
    ...
    audit = {
        ...
        "prior_failure_audit_sha256": protocol["prior_failure_audit_sha256"],
        "predecision_actor": predecision_actor,
        "low_level_actor": actor,
        "development_only": True,
        "counts_as_a0_a7_evidence": False,
        "disjoint_from_formal_p1_p2": True,
    }
