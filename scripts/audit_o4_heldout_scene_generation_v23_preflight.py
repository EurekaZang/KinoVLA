#!/usr/bin/env python3
"""Fail-closed preflight for the stratified v23 O4 scene-acquisition batch."""
...
def main() -> int:
    ...
    checks: dict[str, bool] = {
        ...
        == "kinofail.o4-heldout-scene-generation-v23-freeze.v1",
        ...
        "v21_is_required_and_passed": policy.get("requires_sealed_v21_train_only_pass")
        is True,
        "v22_failure_is_required_and_sealed": policy.get(
            "requires_sealed_v22_acquisition_failure"
        )
        is True,
        "six_fixed_requests": len(scenes) == 6
        and [item.get("scene_id") for item in scenes] == policy.get("fixed_order"),
        "stratified_acquisition": len({item.get("room_type") for item in scenes}) >= 4
        and all(item.get("room_type") != "Bathroom" for item in scenes),
        "one_attempt_no_replacement": policy.get("one_generation_attempt_per_scene")
        is True
        and policy.get("replacement_seed_allowed") is False
        and policy.get("source_repair_allowed") is False,
        "minimum_confirmation_is_three_family": policy.get("minimum_admitted_scenes")
        >= 3
        and policy.get("minimum_admitted_families") >= 3,
        "no_o4_before_batch_adjudication": policy.get(
    ...
    for scene in scenes:
        ...
        checks[f"{slug}_exception_unused"] = not exception.exists()
    ...
    audit = {
        "schema_version": "kinofail.o4-heldout-scene-generation-v23-preflight.v1",
