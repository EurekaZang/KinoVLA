#!/usr/bin/env python3
"""Freeze an excluded O9 high-center escape-program validation D5."""
...
D4 = ROOT / "outputs/kinofail_reconfirmation_a4_v7_checkpoint_tuning_d4/design_audit.json"
D4_SUMMARY = Path(
    "/data/eureka/KinoVLA/outputs/kinofail_reconfirmation_a4_v7_checkpoint_tuning_d4/"
    "corpus/confirm_v2_life_scene_02/"
    "a4v7pilotp3__confirm_v2_life_scene_02__O9_high_centering__moderate/summary.json"
)
OUTPUT = ROOT / "outputs/kinofail_reconfirmation_a4_v7_checkpoint_tuning_d5"
...
def main() -> int:
    ...
    for path in (SOURCE, D4, D4_SUMMARY, COLLECTOR, PRE, POLICY, MANIFEST, FREEZE, SIM):
        if not path.is_file():
            raise FileNotFoundError(path)
    if json.loads(D4_SUMMARY.read_text()).get("passed") is not True:
        raise RuntimeError("D4 did not certify exact F33 source replay")
    rows = [
        row
    ...
    row = rows[0]
    row["pairing"] = "outcome-exposed O9 escape-program validation D5"
    ...
    protocol = {
        "schema_version": "kinofail.reconfirmation-a4-v7-checkpoint-tuning-d5.v1",
        "protocol_id": "kinofail-reconfirmation-a4-v7-checkpoint-tuning-d5",
        ...
        "d4_design_sha256": sha256(D4),
        "d4_summary_sha256": sha256(D4_SUMMARY),
    }
