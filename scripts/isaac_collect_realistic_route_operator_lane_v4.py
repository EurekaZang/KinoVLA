#!/usr/bin/env python3
"""Collect one route-tracking-gated cross-domain realistic O1/O2/O3 lane.

Version 4 adds a fail-closed full-lane tracking and region-coverage contract.
...
def _front_sequence_non_degenerate(captures: list[dict[str, object]]) -> bool:
    ...
    basic_texture_gate = all(
        ...
        and float(item["edge_fraction_gt_0_02"]) >= 0.010
        for item in metrics
    )
    context_gate = all(
...
def main() -> int:
    ...
    try:
        ...
        for step in range(1, int(args.steps) + 1):
            command = route_frame.controller_command(
        ...
        maximum_absolute_route_lateral_offset = max(
        ...
        common_checks = {
            ...
            "full_lane_tracking_within_admitted_budget": (
                ...
                <= admitted_tracking_budget + 1.0e-9
            ),
            "operator_region_sample_count_sufficient": len(region_rows)
            >= int(args.min_region_samples),
        }
        if args.lane == "nominal":
            if args.operator == "o2":
                control_topology = operator_telemetry.get("collision_topology", {})
                central_materials = control_topology.get("central_material_readback", [])
                ...
                lane_checks = {
                    ...
                    "o2_matched_control_segment_layout": control_topology.get("mode")
                    == "segmented_o2_support_under_continuous_pbr"
                    and int(control_topology.get("collision_segment_count", 0)) == 7
                    and int(control_topology.get("central_segment_count", 0)) == 3,
                    "o2_matched_control_is_flat_nominal_material": control_topology.get(
                        "central_surface_state"
                    )
                    == "flat_nominal_material"
                    and len(central_materials) == 3
                    and all(
                        abs(float(material.get(key, -1.0)) - float(nominal_material.get(key, -2.0)))
                        <= 1.0e-6
                        for material in central_materials
        ...
        manifest = {
            "schema_version": (
                f"kinofail.realistic-route-{args.operator}-fresh-app-lane.v4-protocol-candidate"
            ),
            "scene_route_binding": {
            ...
            "collector_lifecycle": "native_v4_tracking_gated_protocol_candidate",
            "lane": args.lane,
            ...
            "route_controller": {
                ...
                "heading_gain": 2.0,
                "lateral_limit_mps": 0.16,
                "admitted_maximum_route_deviation_m": admitted_tracking_budget,
                "minimum_operator_region_samples": int(args.min_region_samples),
