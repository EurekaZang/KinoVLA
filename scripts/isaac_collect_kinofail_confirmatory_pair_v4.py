#!/usr/bin/env python3
"""Confirmatory collector with physical terminal sensing and auditable exits.

This is collector-v3 plus one operational correction: the legacy collector's
unconditional ``os._exit`` is replaced before execution so Python exceptions
and non-zero validation status cannot be silently discarded.
"""
...
def _load_v4_wrapper() -> Any:
    if _sha256(V1_WRAPPER) != EXPECTED_V1_WRAPPER_SHA256:
        raise RuntimeError("confirmatory v4 dependency differs from excluded pilot")
    source = V1_WRAPPER.read_text(encoding="utf-8")
    return_marker = "    implementation.main()\n"
    if source.count(return_marker) != 1:
        raise RuntimeError("v4 wrapper return patch point is not unique")
    source = source.replace(return_marker, "    return implementation.main()\n")
...
'''
    if source.count(marker) != 1:
        raise RuntimeError("v4 fall-horizon insertion point is not unique")
    ...
    if source.count(metadata_marker) != 1:
        raise RuntimeError("v4 provenance insertion point is not unique")
    source = source.replace(metadata_marker, metadata_replacement)

    module = types.ModuleType("kinofail_confirmatory_pair_v4_wrapper")
