#!/usr/bin/env python3
"""Freeze an excluded O8 acquisition pilot for collector-v3 QA."""
...
ARCHIVE = ROOT / "outputs/kinofail_confirmatory_invalid_acquisition_pilot_v1_20260725"
SOURCE_SCHEDULE = (
    ARCHIVE
...
COLLECTOR = ROOT / "scripts/isaac_collect_kinofail_confirmatory_pair_v3.py"
BACKEND = ROOT / "kino_vla/sim/isaac_policy_backend.py"
OUTPUT = ROOT / "outputs/kinofail_acquisition_pilot_v3/design"
GROUP = "cf_190ea2ab6e27fa8cf564"
...
def main() -> int:
    ...
    rows = [
        ...
        if str(row["counterfactual_group_id"]) == GROUP
    ]
    if len(rows) != 2:
        raise RuntimeError("excluded O8 acquisition pilot pair is incomplete")
    ...
    registry = {
        **source_registry,
        "schema_version": "kinofail.acquisition-pilot-registry.v3",
    ...
    protocol["protocol_id"] = "kinofail-acquisition-pilot-v3-physical-terminal-sensing"
    ...
    protocol["allowed"] = {
        ...
        "counterfactual_group_ids": [GROUP],
        ...
        "scene_families": sorted({str(row["scene_family"]) for row in rows}),
        "severity_ids": sorted({str(row["severity_id"]) for row in rows}),
        "target_operators": ["O8_invisible_collider"],
    }
    protocol_path = OUTPUT / "collection_protocol.json"
    _write_json(protocol_path, protocol)

    audit = {
        "schema_version": "kinofail.acquisition-pilot-freeze.v3",
        "created_utc": datetime.now(UTC).isoformat(),
        "status": "frozen_before_excluded_pilot_collection",
        "confirmatory_claim_eligible": False,
        "model_predictions_allowed": False,
        "purpose": [
            "verify that the post-failure horizon advances real Isaac time and sensors",
            "verify minimum temporal support for an excluded early O8 failure",
        ],
        "group_ids": [GROUP],
        "source_sha256": {
            ...
            "isaac_policy_backend": _sha256(BACKEND),
        },
        "artifact_sha256": {
