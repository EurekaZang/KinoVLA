#!/usr/bin/env python3
"""Freeze the minimal A3 O7 reverse-conflict collection before simulation."""
...
def main() -> None:
    schedule = ROOT / "outputs/kinofail_realistic/design_a3_reverse_v1/schedule.jsonl"
    audit_path = ROOT / "outputs/kinofail_realistic/design_a3_reverse_v1/audit.json"
    ...
    output = ROOT / "configs/data/kinofail_realistic_a3_reverse_formal_v1.json"
    if output.exists():
        raise FileExistsError(f"refusing to overwrite {output}")
    rows = [json.loads(line) for line in schedule.read_text(encoding="utf-8").splitlines() if line]
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    if audit.get("passed") is not True or len(rows) != 18:
        raise RuntimeError("A3 reverse design audit did not pass")
    protocol = {
        ...
        "benchmark_id": "kinofail_realistic_a3_reverse_v1",
        "scope": "A3 three-domain O7 reverse-conflict extension: hazard-looking scanned PBR appearance paired with nominal physics; A8 excluded.",
        ...
        "allowed": {
            ...
            "target_operators": ["O7_visual_remap"],
        ...
        "collection_contract": {
            "physical_episodes": 18,
            "counterfactual_pairs": 9,
            "independent_scene_clusters": 9,
            "domains": 3,
            "appearance_views_per_episode": 3,
            "nominal_probe_requires_mu_s_ge": 0.8,
            "nominal_probe_requires_mu_d_ge": 0.6,
            "nominal_probe_requires_depth_bias_m": 0.0,
    ...
    print(json.dumps({"output": str(output), "sha256": _sha(output), "pairs": 9}, indent=2))
