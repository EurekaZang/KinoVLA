#!/usr/bin/env python3
"""Collect and verify the 18-pair O3/O8 temporal-window repair overlay."""
...
PROTOCOL = ROOT / "configs/data/kinofail_realistic_temporal_repair_formal_v2.json"
COLLECTOR = ROOT / "scripts/isaac_collect_kinofail_realistic_temporal_repair_v2.py"
CORPUS = ROOT / "outputs/kinofail_realistic/corpus_scale_v8_temporal_repair_v2"
...
def _accepted(module, pair_id: str, protocol_id: str) -> tuple[bool, list[str]]:
    ...
    for result in results:
        ...
        expected_profile = 1 if operator == "O3_collapse" else 3
        if operator not in {"O3_collapse", "O8_invisible_collider"}:
            issues.append(f"operator_outside_repair_scope:{operator}")
        if (
    ...
    if len(nuisances) == 2 and (
        ...
        or nuisances[0].get("profile_index") != expected_profile
