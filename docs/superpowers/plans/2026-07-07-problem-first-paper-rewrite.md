...
Selective prediction is evaluated as a falsifiable recovery decision. The deployed residual ranks B5-conflict-bi attribution errors with AUROC $0.869$, but the corpus contains no $\theta$ outside training support. Under the frozen appearance split, the A5 broad test always-safe cost is $1.52$; the minimum nonzero residual policy covers $0.0533$ at cost $1.84$ and is significantly worse than safe. A6 brackets the privileged O10 base transition at $[0.25,0.30]$; $0.2754$ is descriptive only. Residual failure AUROC is $1.0$ over five floor groups, but exact permutation $p=0.10$, and all evaluated learned agents intervene at every floor. These are boundary/negative results, not calibrated-abstention or learned-flip claims.
...
All reported Paper-A evidence is from the Isaac Go2 physics-simulation stack, not from outdoor hardware deployment. The evidence scope ends at A7. The operators are constructed to isolate scientific questions, so they simplify real terrain physics even when they are physically motivated. The forced-label matrix measures causal recovery consequence under registered scripted primitives; it does not claim that a fully autonomous navigation stack has been optimized end-to-end.
