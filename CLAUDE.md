...
- **A8 IN PROGRESS → publication-partial (external validity).** Dual-track design+pipeline landed. **A8a real Qwen3-VL-4B (official images):** ZS UR5 **0.529**, BDV2 **0.629**, RoboFail **0.771**, RoboVQA **0.843**; FailCoT-SFT seed0 (BDV2-train 2000, 2ep, best val 0.0118) → 0.514/0.616/0.771/0.838 (Δ −0.015/−0.013/0/−0.005, honest null). Guardian-8B paper ref only: 0.86/0.77/0.85. **A8b real multi-sensory CLAIM-SUPPORTING:** REFLECT admission PASS (30 eps); strata E2=17/E3=13/E4=30 from task metadata + early-nominal windows. CBA=½(E2+E3): **V-only 0.738 < P-only 1.000 = fusion 1.000**; fusion overall 0.933 recovers E4 (0.867) while vision E4=0.0; ΔP−V on E3 **+0.231**, McNemar P≻V **p≈0**. Δ_conflict≈0 (P ceiling). Multi-seed SFT running. No surrogate proprio. See `A实验/A8.md`, `outputs/eval/a8/`.
...
2026-07-11 | A8 external-validity real battery: A8a ZS 0.529/0.629/0.771/0.843 + FailCoT-SFT seed0 null OOD deltas; A8b REFLECT multi-strata E2/E3/E4=17/13/30, CBA V 0.738 < P/fusion 1.000, McNemar P≻V p≈0, fusion recovers E4 0.867. | A实验/A8.md; outputs/eval/a8/; scripts/a8_*.py; kino_vla/eval/a8_reflect.py
```
...
A8 SCOPE — claim-supporting partial complete. C4: public ZS competitive (RoboVQA 0.843≈Guardian 0.85). C2 external: multi-strata REFLECT shows vision insufficient (CBA 0.738) while proprio/fusion solve conflict (CBA 1.000) and fusion recovers nominal E4 (0.867 vs V 0.0); McNemar P≻V significant. Remaining: multi-seed SFT (running), end-to-end latent Kino-Projector adapters on arm state, optional RLBench/sim_data scale-up. Δ_conflict≈0 is P-ceiling scope, not a surrogate. No surrogate proprio.
Pending experiments — finish multi-seed FailCoT; optional latent-adapter A8b SFT; optional RLBench.
```
