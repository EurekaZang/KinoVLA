...
- **A8 IN PROGRESS (external validity; not Go2 hardware A8d).** Dual-track design+pipeline landed. **A8a real Qwen3-VL-4B (official images):** zero-shot UR5 **0.529**, BDV2 **0.629**, RoboFail **0.771**, RoboVQA **0.843**; FailCoT-SFT (BDV2-train 2000, 2ep, best val 0.0118) → UR5 **0.514** (Δ−0.015), BDV2 **0.616** (Δ−0.013), RoboFail **0.771** (Δ0), RoboVQA **0.838** (Δ−0.005). Guardian-8B paper ref only: RoboFail 0.86 / UR5 0.77 / RoboVQA 0.85. **A8b unblocked:** REFLECT real multi-sensory admission PASS (30 eps; joint/gripper/force/EE + RGB JPEG-XL). Failure-only E3-dominant: V-only **0.967**, P-only **1.000**, fusion **1.000**; Δ proprio−vision E3=+0.033 (McNemar n.s.). CBA deferred without E2/success strata. No surrogate proprio. See `A实验/A8.md`, `outputs/eval/a8/`.
...
2026-07-11 | A8 external-validity: real ZS table UR5/BDV2/RoboFail/RoboVQA 0.529/0.629/0.771/0.843; FailCoT-SFT seed0 trained (n=2000,2ep) eval deltas −0.015/−0.013/0/−0.005; REFLECT A8b admission PASS (30 eps), E3 P 1.000 vs V 0.967. | A实验/A8.md; outputs/eval/a8/; scripts/a8_*.py
```
...
A8 SCOPE — IN PROGRESS / partial claim support. A8a public ZS is real and competitive on RoboVQA/RoboFail vs Guardian paper refs; FailCoT-SFT (single seed, BDV2-train 2k) does not improve OOD over ZS (honest null/slight regression). A8b multi-sensory admission real, but failure-only E3 corpus makes CBA/Δ_conflict undefined — need E2/success strata + trained latent/conflict adapters before claim-grade C2 external matrix. RLBench full table and multi-seed SFT still open. No surrogate proprio.
Pending experiments — A8b E2/success strata (sim_data or negatives); multi-seed FailCoT/Kino-General; optional RLBench.
```
